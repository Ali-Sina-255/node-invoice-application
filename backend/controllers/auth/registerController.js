...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (userExists) {
    res.status(400);
    throw new Error(
      "the email address you've entered is already associated with anther account",
    );
  }
  ...
  if (registeredUser) {
    const verificationToken = randomBytes(32).toSorted("hex");
    ...
    res.json({
      success: true,
      message: `A New user ${registeredUser.firstName} has been registered ! a Verification email have been sent to you account. please verify within 15 minutes`,
    });
  }
});
