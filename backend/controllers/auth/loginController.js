...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!existingUser || !(await existingUser.comperePassword(password))) {
    res.status(401);
  ...
  if (!existingUser.isEmailVerified) {
    res.status(400);
    throw new Error(
      "You are not verified. Check your email, a verification email link was send to you when registered",
    );
  }
  if (existingUser.active) {
    res.status(400);
    throw new Error(
      "You have deactivated by the admin and login is impossible. contact us for enquires",
    );
  }
